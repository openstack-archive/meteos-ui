...
function run_pep8 {
  ...
  DJANGO_SETTINGS_MODULE=meteos_ui.test.settings ${command_wrapper} flake8
}
...
function run_pep8_changed {
    ...
    warn_on_flake8_without_venv
    diff -u --from-file /dev/null ${files} | DJANGO_SETTINGS_MODULE=meteos_ui.test.settings ${command_wrapper} flake8 --diff
    exit
}

function run_sphinx {
    echo "Building sphinx..."
    DJANGO_SETTINGS_MODULE=meteos_ui.test.settings ${command_wrapper} python setup.py build_sphinx
    echo "Build complete."
}
...
function run_tests {
  ...
  if [ $with_selenium -eq 0 -a $integration -eq 0 ]; then
      testopts="$testopts --exclude-dir=meteos_ui/test/integration_tests"
  fi
...
function run_tests_all {
  echo "Running meteos UI tests"
  export NOSE_XUNIT_FILE=meteos_ui/nosetests.xml
  if [ "$NOSE_WITH_HTML_OUTPUT" = '1' ]; then
    export NOSE_HTML_OUT_FILE='dashboard_nose_results.html'
  fi
  ${command_wrapper} ${coverage_run} $root/manage.py test meteos_ui --settings=openstack_dashboard.test.settings $testopts
...
function run_makemessages {
  ...
  echo -n "meteos ui javascript: "
  babel_extract djangojs
  meteos_JS_RESULT=$?

  cd ..
  if [ $check_only -eq 1 ]; then
    rm meteos_ui/locale/django*.pot
  fi

  exit $(($meteos_PY_RESULT || $meteos_JS_RESULT))
}
